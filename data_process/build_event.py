...
def gen_builders(type_features, text_map, build_des_file, feature_des_file):
    ...
    f.close()
    fea_f.close()
    logging.info("event_dim = %d", event_dim)
    logging.info("feature_dim = %d", feature_dim)
    return builders
...
if __name__ == '__main__':
    ...
    if not os.path.exists(event_dir):
        os.mkdir(event_dir)
    for filepath in glob.glob(original_data_dir + "/*tsv"):
        name = os.path.basename(filepath)
        build_event(filepath, builders)
    logging.info("#TimeDuration < 0 error = %d", TimeFeatureExtractor.nerror)
